...
long long unsigned int timecode_kkkk_amanda;
...
class DemoCallback : public Callback {
 public:
  ...
  Status OnClusterBegin(const ElementMetadata& metadata, const Cluster& cluster,
    ...
    timecode_kkkk_amanda = cluster.timecode.value();
...
int main_no_more(int argc, char* argv[]) {
  ...
  std::cout << " for your pleasure " << timecode_kkkk_amanda << "\n";
...
int64_t my_amanda_main(FILE* ric_the_guy) {
  timecode_kkkk_amanda = 1;

  FILE* file = ric_the_guy;
  ...
  // std::cout << " again for your pleasure -> " << timecode_kkkk_amanda <<
  // "\n";

  return timecode_kkkk_amanda;
}

int __fastcall kkk_get_damaged_webm_duration_by_bhond(FILE* amanda_file) {
  return my_amanda_main(amanda_file);
}

int main_no(int argc, char** argv) {
  FILE* myfile_amanda = fopen(argv[1], "rb");
  kkk_get_damaged_webm_duration_by_bhond(myfile_amanda);
  fclose(myfile_amanda);
  printf("Te amo...");
  return 0;
}
}