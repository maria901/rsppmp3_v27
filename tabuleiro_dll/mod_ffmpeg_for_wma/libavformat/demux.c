                      ...
                      AVDictionary **options)
{
     ...
     if (s->pb)
     {
          ...
          if (!s->iformat)
          {
               pedro_dprintf(0, "Error 151\n");
               return av_probe_input_buffer2(s->pb, &s->iformat, filename,
                                             s, 0, s->format_probesize);
          }
          else if (s->iformat->flags & AVFMT_NOFILE)
          {
               pedro_dprintf(0, "Custom AVIOContext makes no sense and "
                                "will be ignored with AVFMT_NOFILE format.\n");
               av_log(s, AV_LOG_WARNING, "Custom AVIOContext makes no sense and "
                                         "will be ignored with AVFMT_NOFILE format.\n");
          }
          pedro_dprintf(0, "Error 161\n");
          return 0;
     }
     ...
     {
          pedro_dprintf(0, "Error 162\n");
          return score;
     }

     if ((ret = s->io_open(s, &s->pb, filename, AVIO_FLAG_READ | s->avio_flags, options)) < 0)
     {
          pedro_dprintf(0, "Error 163\n"); 
          return ret;
     }

     if (s->iformat)
     {
          pedro_dprintf(0, "Error 164\n"); 
          return 0;
     }
     pedro_dprintf(0, "Error 165\n"); 
     return av_probe_input_buffer2(s->pb, &s->iformat, filename,
                                   s, 0, s->format_probesize);
}
                        ...
                        const AVInputFormat *fmt, AVDictionary **options)
{
     ...
     {
          pedro_dprintf(0, "Error 465768\n");
          return AVERROR(ENOMEM);
     }
     si = ffformatcontext(s);
     if (!s->av_class)
     {
          pedro_dprintf(0, "Input context has not been properly allocated by avformat_alloc_context() and is not NULL either\n");
     ...
     {
          pedro_dprintf(0, "Error ftgryh\n");
          goto fail;
     }

     if (!(s->url = av_strdup(filename ? filename : "")))
     {
          pedro_dprintf(0, "Error cfvdgb\n");
          ret = AVERROR(ENOMEM);
          goto fail;
     }
     ...
     {
          pedro_dprintf(0, "Discarding ID3 tags because more suitable tags were found\n");
...
static int probe_codec(AVFormatContext *s, AVStream *st, const AVPacket *pkt)
{
     ...
     if (sti->request_probe > 0)
     {
          ...
          end = si->raw_packet_buffer_remaining_size <= 0 || sti->probe_packets <= 0;
...
void ff_rfps_calculate(AVFormatContext *ic)
{
     for (unsigned i = 0; i < ic->nb_streams; i++)
     {
          ...
          if (sti->info->duration_count > 1 && !st->r_frame_rate.num && tb_unreliable(sti->avctx))
          {
               int num = 0;
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
     ...
     for (unsigned i = 0; i < ic->nb_streams; i++)
     {
          ...
          if (avctx->codec_type == AVMEDIA_TYPE_VIDEO)
          {
               ...
               if (!st->r_frame_rate.num)
               {
                    if (avctx->time_base.den * (int64_t)st->time_base.num <= avctx->time_base.num * (uint64_t)avctx->ticks_per_frame * st->time_base.den)
                    {
