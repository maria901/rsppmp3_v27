...
const AVClass *ff_urlcontext_child_class_iterate(void **iter)
{
    ...
    for (i = (uintptr_t)*iter; url_protocols[i]; i++) {
		pedro_dprintf(0, "loop ff_urlcontext_child_class_iterate -> %s\n", url_protocols[i]->name);
...
const char *avio_enum_protocols(void **opaque, int output)
{
	...
	pedro_dprintf(0, "dentro de avio_enum_protocols \n");
...
pedro_dprintf(0, "protocolo nome %s\n", name);	

    for (i = 0; url_protocols[i]; i++) {
        if (!strcmp(url_protocols[i]->name, name))
		{
			pedro_dprintf(0, "achou protocolo \n");
            return url_protocols[i]->priv_data_class;
		}
    }
	pedro_dprintf(0, "nao achou protocolo \n");
    return NULL;
}
