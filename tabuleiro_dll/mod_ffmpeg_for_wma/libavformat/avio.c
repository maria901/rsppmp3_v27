...
static const struct URLProtocol *url_find_protocol(const char *filename)
{
    ...
    pedro_dprintf(0, "ricardo inside, function -> url_find_protocol\n");
        ...
        is_dos_path(filename))
    {
        ...
        pedro_dprintf(0, "ricardo inside, modo 1\n");
    ...
    else
    {
        ...
        pedro_dprintf(0, "ricardo inside, modo 2\n");
    }
    ...
    if (!protocols)
    {
        pedro_dprintf(0, "ricardo inside, error 240\n");
        return NULL;
    }
    for (i = 0; protocols[i]; i++)
    {
        ...
        pedro_dprintf(0, "I love this job -> .%s. .%s.\n", proto_str, up->name);

        if (!strcmp(proto_str, up->name))
        {
            av_freep(&protocols);

            pedro_dprintf(0, "ricardo inside, achou protocolo\n");
            ...
            !strcmp(proto_nested, up->name))
        {
            ...
            pedro_dprintf(0, "ricardo inside, achou 2 protocolo\n");
            av_freep(&protocols);
            return up;
        }
    }
    ...
    pedro_dprintf(0, "ricardo inside, error 241\n");
                ...
                const AVIOInterruptCB *int_cb)
{
    ...
    pedro_dprintf(0, "ricardo inside, function -> ffurl_alloc\n");

    p = url_find_protocol(filename);
    if (p)
    {
        pedro_dprintf(0, "ricardo inside, vai tentar alocar\n");
        return url_alloc_for_protocol(puc, p, filename, flags, int_cb);
    }

    *puc = NULL;

    pedro_dprintf(0, "ricardo inside, error 210\n");
                         ...
                         URLContext *parent)
{
    ...
    if (ret < 0)
    {
        pedro_dprintf(0, "ricardo inside, error 180\n");
        return ret;
    }
    if (parent)
    {
        ...
        if (ret < 0)
        {
            pedro_dprintf(0, "ricardo inside, error 181 fail here\n");
            goto fail;
        }
    }
    if (options &&
    ...
    {
        pedro_dprintf(0, "ricardo inside, error 182 fail here\n");
        goto fail;
    }
    if (options && (*puc)->prot->priv_data_class &&
        (ret = av_opt_set_dict((*puc)->priv_data, options)) < 0)
    {
        pedro_dprintf(0, "ricardo inside, error 183 fail here\n");
        goto fail;
    }
    ...
    {
        pedro_dprintf(0, "ricardo inside, error 184 fail here\n");
        goto fail;
    }

    if ((ret = av_dict_set(options, "protocol_blacklist", blacklist, 0)) < 0)
    {
        pedro_dprintf(0, "ricardo inside, error 185 fail here\n");
        goto fail;
    }

    if ((ret = av_opt_set_dict(*puc, options)) < 0)
    {
        pedro_dprintf(0, "ricardo inside, error 186 fail here\n");
        goto fail;
    }
    ...
    if (!ret)
    {
        pedro_dprintf(0, "ricardo inside, error -> no error\n");
        return 0;
    }
fail:
    ffurl_closep(puc);
    pedro_dprintf(0, "ricardo inside, error 190 fail here\n");
    return ret;
}
