...
III_sideinfo(struct mad_bitptr *ptr, unsigned int nch,
             ...
             unsigned int *data_bitlen, unsigned int *priv_bitlen)
{
...
III_requantize(unsigned int value, signed int exp)
{
     ...
     if (exp < 0)
     {
          if ((int)-exp >= (int)(sizeof(mad_fixed_t) * CHAR_BIT))
          {
               /* underflow */
               requantized = 0;
          }
...
III_huffdecode(struct mad_bitptr *ptr, mad_fixed_t xr[576],
               struct channel *channel,
               unsigned char const *sfbwidth, unsigned int part2_length)
{
     ...
     /* big_values */
     {
          ...
          while (big_values-- && cachesz + bits_left > 0)
          {
               ...
               while (!pair->final)
               {
                    ...
                    pair =
                        &table[pair->ptr.offset + MASK(bitcache, cachesz, clumpsz)];
               }
               ...
               for (sfbi = l = 0; l < 576; ++sfbi, l += n)
               {
                    ...
                    for (i = 0; i < n; ++i)
                    {
                         ...
                         else
                         {
                              ...
                              opposite =
                                  mad_f_mul(left, lsf_scale[(is_pos - 1) / 2]);
           ...
           struct sideinfo *si, unsigned int nch)
{
     ...
     for (gr = 0; gr < ngr; ++gr)
     {
          ...
          for (ch = 0; ch < nch; ++ch)
          {
               ...
               {
                    part2_length =
                        III_scalefactors(ptr, channel, &si->gr[0].ch[ch],
                                         gr == 0 ? 0 : si->scfsi[ch]);
               }

               error =
                   III_huffdecode(ptr, xr[ch], channel, sfbwidth[ch], part2_length);
               if (error)
                    return error;
          }
