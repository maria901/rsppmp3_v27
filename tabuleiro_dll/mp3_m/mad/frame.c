...
extern int madoldvalue;
double madmpegversion = 1.0;
extern int mad_mp3_is_valid;
int mad_force_exit;
int mad_old_value_b = 0;
static int mad_counter = 0;
...
decode_header(struct mad_header *header, struct mad_stream *stream)
{
     ...
     if (header->flags & MAD_FLAG_LSF_EXT)
     {
          ...
          if (header->flags & MAD_FLAG_MPEG_2_5_EXT)
               header->samplerate /= 2, madmpegversion = 2.5;
     }
     ...
     if (0 == mad_mp3_is_valid)
     {
          ...
          if (-1 == mad_old_value_b)
          {
               mad_old_value_b = header->samplerate + header->mode;
          }
          else
          {
               if ((int)mad_old_value_b != (int)(header->samplerate + header->mode))
               {
                    mad_counter++;
                    mad_old_value_b = header->samplerate + header->mode;
               }
     ...
     {
          static int oldvalue2 = -1;
          if (-1 == madoldvalue)
          {
               oldvalue2 = -1;

               madoldvalue = header->samplerate + header->mode;
          }
          else
          {
               if (-1 == oldvalue2)
               {
                    oldvalue2 = madoldvalue;
                    madoldvalue = header->samplerate + header->mode;
               }
               else
               {
                    if ((int)(header->samplerate + header->mode) == (int)madoldvalue && (int)(header->samplerate + header->mode) == (int)oldvalue2)
                    {
                         mad_mp3_is_valid = 1;
                    }
                    else
...
int mad_header_decode(struct mad_header *header, struct mad_stream *stream)
{
     ...
     if (stream->skiplen)
     {
          ...
          if ((int)(end - ptr) < (int)stream->skiplen)
          {
