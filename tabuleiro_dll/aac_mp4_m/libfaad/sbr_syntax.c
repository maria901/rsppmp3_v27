                           ...
                           uint8_t psResetFlag)
{
...
#endif
    {
        ...
        {
            sbr->bs_sbr_crc_bits = (uint16_t)faad_getbits(ld, 10 DEBUGVAR(1, 199, "sbr_bitstream(): bs_sbr_crc_bits"));
        }
    }
...
static uint8_t sbr_single_channel_element(bitfile *ld, sbr_info *sbr)
{
    ...
    {
        faad_getbits(ld, 4 DEBUGVAR(1, 221, "sbr_single_channel_element(): bs_reserved_bits_data"));
    }
    ...
    if (sbr->bs_extended_data)
    {
        ...
        if (cnt == 15)
        {
            cnt += (uint16_t)faad_getbits(ld, 8 DEBUGVAR(1, 226, "sbr_single_channel_element(): bs_esc_count"));
        }
...
static uint8_t sbr_channel_pair_element(bitfile *ld, sbr_info *sbr)
{
    ...
    if (sbr->bs_extended_data)
    {
        ...
        if (cnt == 15)
        {
            cnt += (uint16_t)faad_getbits(ld, 8 DEBUGVAR(1, 235, "sbr_channel_pair_element(): bs_esc_count"));
        }
...
static uint8_t sbr_grid(bitfile *ld, sbr_info *sbr, uint8_t ch)
{
    ...
    case FIXVAR:
        ...
        {
            sbr->bs_rel_bord[ch][rel] = 2 * (uint8_t)faad_getbits(ld, 2 DEBUGVAR(1, 253, "sbr_grid(): bs_rel_bord")) + 2;
        }
    ...
    case VARFIX:
        ...
        for (rel = 0; rel < bs_num_env - 1; rel++)
        {
            sbr->bs_rel_bord[ch][rel] = 2 * (uint8_t)faad_getbits(ld, 2 DEBUGVAR(1, 258, "sbr_grid(): bs_rel_bord")) + 2;
        }
        i = sbr_log2(bs_num_env + 1);
        sbr->bs_pointer[ch] = (uint8_t)faad_getbits(ld, i
    ...
    case VARVAR:
        ...
        {
            sbr->bs_rel_bord_0[ch][rel] = 2 * (uint8_t)faad_getbits(ld, 2 DEBUGVAR(1, 265, "sbr_grid(): bs_rel_bord")) + 2;
        }
        for (rel = 0; rel < sbr->bs_num_rel_1[ch]; rel++)
        {
            sbr->bs_rel_bord_1[ch][rel] = 2 * (uint8_t)faad_getbits(ld, 2 DEBUGVAR(1, 266, "sbr_grid(): bs_rel_bord")) + 2;
        }
...
static void invf_mode(bitfile *ld, sbr_info *sbr, uint8_t ch)
{
    ...
    for (n = 0; n < sbr->N_Q; n++)
    {
        sbr->bs_invf_mode[ch][n] = (uint8_t)faad_getbits(ld, 2 DEBUGVAR(1, 271, "invf_mode(): bs_invf_mode"));
    }
}
...
#endif
    default:
        sbr->bs_extension_data = (uint8_t)faad_getbits(ld, 6 DEBUGVAR(1, 279, "sbr_single_channel_element(): bs_extension_data"));
        return 6;
    }
}
