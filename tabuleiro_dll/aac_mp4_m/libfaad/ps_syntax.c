...
uint16_t ps_data(ps_info *ps, bitfile *ld, uint8_t *header)
{
    ...
    *header = 0;
    ...
    if (ps->frame_class)
    {
        ...
        {
            ps->border_position[n] = (uint8_t)faad_getbits(ld, 5 DEBUGVAR(1, 1008, "ps_data(): border_position")) + 1;
        }
    }
    ...
    if (ps->enable_ext)
    {
        ...
        if (cnt == 15)
        {
            cnt += (uint16_t)faad_getbits(ld, 8 DEBUGVAR(1, 1012, "ps_data(): esc_count"));
        }
