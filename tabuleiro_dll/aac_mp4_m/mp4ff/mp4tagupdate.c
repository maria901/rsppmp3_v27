...
unsigned membuffer_transfer_from_file(membuffer *buf, mp4ff_t *src, unsigned bytes)
{
	...
	if ((unsigned)mp4ff_read_data(src, (void *)((char *)bufptr + oldsize), bytes) != bytes)
	{
...
static uint32_t find_atom(mp4ff_t *f, uint64_t base, uint32_t size, const char *name)
{
	...
	for (;;)
	{
		...
		if (atom_size > remaining || atom_size < 8)
			break;
		mp4ff_read_data(f, (void *)atom_name, 4);
...
static uint32_t modify_moov(mp4ff_t *f, const mp4ff_metadata_t *data, void **out_buffer, uint32_t *out_size)
{
	...
	else
	{
		...
		*out_buffer = malloc(*out_size);
		...
		mp4ff_set_position(f, total_base);
		mp4ff_read_data(f, (void *)p_out, (uint32_t)(udta_offset - total_base));
		p_out += (uint32_t)(udta_offset - total_base);
		*(uint32_t *)p_out = fix_byte_order_32(mp4ff_read_int32(f) + size_delta);
		p_out += 4;
		mp4ff_read_data(f, (void *)p_out, 4);
		p_out += 4;
		mp4ff_read_data(f, (void *)p_out, (uint32_t)(meta_offset - udta_offset - 8));
		p_out += (uint32_t)(meta_offset - udta_offset - 8);
		*(uint32_t *)p_out = fix_byte_order_32(mp4ff_read_int32(f) + size_delta);
		p_out += 4;
		mp4ff_read_data(f, (void *)p_out, 4);
		p_out += 4;
		mp4ff_read_data(f, (void *)p_out, (uint32_t)(ilst_offset - meta_offset - 8));
		...
		*(uint32_t *)p_out = fix_byte_order_32(mp4ff_read_int32(f) + size_delta);
		...
		mp4ff_read_data(f, (void *)p_out, (uint32_t)(total_size - (ilst_offset - total_base) - ilst_size));
