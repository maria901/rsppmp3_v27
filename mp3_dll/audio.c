...
//sligthly modified by BinaryWork Corp. in 2021
...
audio_file *
open_audio_file(char *infile, int samplerate, int channels,
...
static int
write_wav_header(audio_file *aufile)
{
     ...
     *p++ = 'F';
     ...
     word32 = (data_size + (44 - 8) < (float)MAXWAVESIZE) ? (unsigned long)data_size + (44 - 8) : (unsigned long)MAXWAVESIZE;
     ...
     *p++ = 'a';
     ...
     word32 = data_size < MAXWAVESIZE ? (unsigned long)data_size : (unsigned long)MAXWAVESIZE;
     *p++ = (unsigned char)(word32 >> 0);
     *p++ = (unsigned char)(word32 >> 8);
     *p++ = (unsigned char)(word32 >> 16);
     *p++ = (unsigned char)(word32 >> 24);
...
static int
write_audio_16bit(audio_file *aufile, void *sample_buffer,
                  unsigned int samples)
{
     ...
     char *data =
         (char *)malloc(samples * aufile->bits_per_sample * sizeof(char) / 8);
     ...
     {
          ret =
              fwrite(data, samples, aufile->bits_per_sample / 8, aufile->sndfile);
     }
     if (data)
          free(data);
     return ret;
...
static int
write_audio_24bit(audio_file *aufile, void *sample_buffer,
                  unsigned int samples)
{
     int ret;
     unsigned int i;
...
static int
write_audio_32bit(audio_file *aufile, void *sample_buffer,
                  unsigned int samples)
{
     int ret;
     unsigned int i;
...
static int
write_audio_float(audio_file *aufile, void *sample_buffer,
                  unsigned int samples)
{
     int ret;
     unsigned int i;
     ...
     unsigned char *data =
         (unsigned char *)malloc(samples * aufile->bits_per_sample *
                                 sizeof(char) / 8);
