...
encryptfile (char *inputfile, char *outputfile, char *password)
{
	...
	char *buffer = NULL;
	int returnvalue = 0;
	if ((myfile = _wfopen (permissive_name_m_(amanda_utf8towide_3_(inputfile)), L"rb")) == NULL)
	{
		returnvalue = 1;
		goto saida;
	}
	if ((outfile = _wfopen (permissive_name_m_(amanda_utf8towide_3_(outputfile)), L"wb")) == NULL)
	{
		returnvalue = 2;
		goto saida;
	}
