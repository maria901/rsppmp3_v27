...
//sligthly modified by smartape from Boqueirão in 2021
...
/*-
 * IMPLEMENTATION NOTES. (not by smartape)
 *
...
#include /* amanda's smart ape */        "sha.h"

#include /* amanda's smart ape */ "mv_from_be.h"
...
void get_sha_512_as_string_k(char *buffer_k, unsigned int len_k, char * digest_k)
{
	...
	char temp_out_k[SHA512_DIGEST_LENGTH] ={0};
	memcpy(temp_out_k, SHA512((void *) buffer_k, len_k, NULL), SHA512_DIGEST_LENGTH);
	for(i_k = 0; i_k < SHA512_DIGEST_LENGTH; i_k++)
	{
		sprintf((char *) ola_k, "%02x", (int) (unsigned char)temp_out_k[i_k]);
		ola_k += 2;
	}
}
