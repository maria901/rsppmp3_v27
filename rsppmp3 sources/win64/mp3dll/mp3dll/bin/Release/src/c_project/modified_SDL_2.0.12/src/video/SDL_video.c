...
{
	pedro_dprintf(-1, "dentro de SDL_HideWindow 1");
	CHECK_WINDOW_MAGIC(window,);

	if (!(window->flags & SDL_WINDOW_SHOWN)) {
		return;
	}
	pedro_dprintf(-1, "dentro de SDL_HideWindow 2");
	window->is_hiding = SDL_TRUE;
	SDL_UpdateFullscreenMode(window, SDL_FALSE);
	pedro_dprintf(-1, "dentro de SDL_HideWindow 3");
	if (_this->HideWindow) {
		_this->HideWindow(_this, window);
	}
	pedro_dprintf(-1, "dentro de SDL_HideWindow 4");
	window->is_hiding = SDL_FALSE;
	SDL_SendWindowEvent(window, SDL_WINDOWEVENT_HIDDEN, 0, 0);
	pedro_dprintf(-1, "dentro de SDL_HideWindow 5");
}
...
{
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow");

	SDL_VideoDisplay *display;

	CHECK_WINDOW_MAGIC(window,);
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 1");
	window->is_destroying = SDL_TRUE;

	if(1 == window_created_kp)
	{
		pedro_dprintf(-1, "dentro de SDL_DestroyWindow, vai esconder");
		/* Restore video mode, etc. */
		SDL_HideWindow(window);
	}
	else
	{
		pedro_dprintf(-1, "dentro de SDL_DestroyWindow, nao vai esconder");
	}

	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 2");
	...
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 3");
	...
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 4");
	if (window->surface) {
		window->surface->flags &= ~SDL_DONTFREE;
		SDL_FreeSurface(window->surface);
	}
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 5");
	if (_this->DestroyWindowFramebuffer) {
		_this->DestroyWindowFramebuffer(_this, window);
	}
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 6");
	if (_this->DestroyWindow) {
		_this->DestroyWindow(_this, window);
	}
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 7");
	if (window->flags & SDL_WINDOW_OPENGL) {
		SDL_GL_UnloadLibrary();
	}
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 8");
	if (window->flags & SDL_WINDOW_VULKAN) {
		SDL_Vulkan_UnloadLibrary();
	}
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 9");
	display = SDL_GetDisplayForWindow(window);
	if (display->fullscreen_window == window) {
		display->fullscreen_window = NULL;
	}
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 10");
	...
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 11");
	while (window->data) {
	...
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 12");
	...
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 13");
	SDL_free(window);
	pedro_dprintf(-1, "dentro de SDL_DestroyWindow 14");
}
