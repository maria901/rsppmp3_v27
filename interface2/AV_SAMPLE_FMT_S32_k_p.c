...
modified 22-October-2020 06:42 AM to use the following code
...
if(3 > mv_______->libav_c___original_number_of_channels_k_p)
{
	...
	int i_k_p;
	int * ar_buffer_ = (int *) decoded_frame->data[0];
	...
	{
		ar_buffer__a_little_more[i_k_p] = (int16_t) (ar_buffer_[i_k_p] >> 16);
	}
...
else if(6 == mv_______->libav_c___original_number_of_channels_k_p)
{
	...
	*size_out=outputBufferLen;
	...
	int32_t * ar_buffer_ = (int32_t *) decoded_frame->data[0];

	for(i=0; i < number_of_samples_k_p; i++)
	{
		//channel 0
		int32_t sample_k_p = ar_buffer_[ricardo_deslocador__++] >> 16;
		...
		///////////////////////////////////////////////
		//channel 1
		sample_k_p = ar_buffer_[ricardo_deslocador__++] >> 16;
...
else //other combinations of channels that are not 5.1 or 7.1
{
	...
	for(i_k_p = 0; i_k_p < (data_size/4); i_k_p++)//simply ignore the additional channels
	{
		...
		{
			ar_buffer__a_little_more[i_desloca_k_p] = (int16_t) (ar_buffer_[i_k_p] >> 16);
			i_desloca_k_p++;
		}
